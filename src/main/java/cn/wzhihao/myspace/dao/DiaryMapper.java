...
public interface DiaryMapper extends Mapper<Diary> {
    ...
    @Update("update myspace_diary set diary_title=#{diaryTitle}, diary_body=#{diaryBody}, update_time=#{updateTime} where id=#{id} and user_email=#{userEmail}")
    int updateByEmailAndId(Diary diary);

    @Select("select FROM_UNIXTIME(create_time/1000,'%Y年%m月') months,\n" +
            "COUNT(id) count FROM `myspace_diary` where user_email=#{email}\n" +
            "GROUP BY months ORDER BY months DESC")
    List<Map<String,Integer>> selectArchiveByEmail(String email);
